...
///
/// Directives that require non-default Cargo features are tagged with
/// their required feature(s) and gracefully skipped when those features
/// are not compiled in — ensuring the test passes under any valid
/// feature combination (default, --all-features, --no-default-features).
#[test]
fn test_parser_comprehensive_directive_coverage() {
    // Helper to check whether a given Cargo feature is compiled in.
    // We rely on cfg! which is evaluated at compile time.
    fn feature_enabled(feature: &str) -> bool {
        match feature {
            "dnssec" => cfg!(feature = "dnssec"),
            "dbus" => cfg!(feature = "dbus"),
            "ubus" => cfg!(feature = "ubus"),
            "conntrack" => cfg!(feature = "conntrack"),
            "nftset" => cfg!(feature = "nftset"),
            "ipset" => cfg!(feature = "ipset"),
            "dhcp" => cfg!(feature = "dhcp"),
            "dhcp6" => cfg!(feature = "dhcp6"),
            "tftp" => cfg!(feature = "tftp"),
            "auth" => cfg!(feature = "auth"),
            "script" => cfg!(feature = "script"),
            "loop-detect" => cfg!(feature = "loop-detect"),
            "dumpfile" => cfg!(feature = "dumpfile"),
            "inotify" => cfg!(feature = "inotify"),
            "luascript" => cfg!(feature = "luascript"),
            "idn" => cfg!(feature = "idn"),
            _ => true, // Unknown features assumed available
        }
    }
    ...
    //
    // Each tuple is (directive_line, description, required_feature_or_empty).
    // When required_feature is non-empty, the directive is only tested if
    // that Cargo feature is compiled in; otherwise it is gracefully skipped.
    //
    ...
    let directives: Vec<(&str, &str, &str)> = vec![
        // ── DNS Settings ──
        ("port=5353", "dns port", ""),
        ...
        ("dns-loop-detect", "loop detection", "loop-detect"),
        ...
        ("ipset=/example.com/myset", "ipset", "ipset"),
        ("nftset=/example.com/ip#filter#myset", "nftset", "nftset"),
        ...
        // ── DHCP Settings ──
        ("dhcp-range=192.168.0.50,192.168.0.150,12h", "dhcp range", "dhcp"),
        ("dhcp-host=11:22:33:44:55:66,192.168.0.60", "dhcp host", "dhcp"),
        ("dhcp-option=option:router,192.168.0.1", "dhcp option", "dhcp"),
        ("dhcp-option-force=option:dns-server,192.168.0.1", "dhcp opt force", "dhcp"),
        ("dhcp-boot=/pxelinux.0", "dhcp boot", "dhcp"),
        ("dhcp-match=set:ipxe,175", "dhcp match", "dhcp"),
        ("dhcp-vendorclass=set:msft,MSFT", "dhcp vendorclass", "dhcp"),
        ("dhcp-userclass=set:myclass,myuserclass", "dhcp userclass", "dhcp"),
        ("dhcp-mac=set:mymac,00:60:8C:*:*:*", "dhcp mac", "dhcp"),
        ("dhcp-ignore=tag:known", "dhcp ignore", "dhcp"),
        ("dhcp-ignore-names", "dhcp ignore names", "dhcp"),
        ("dhcp-name-match=set:wpad,wpad", "dhcp name match", "dhcp"),
        ("dhcp-authoritative", "dhcp authoritative", "dhcp"),
        ("dhcp-rapid-commit", "dhcp rapid commit", "dhcp"),
        ("dhcp-lease-max=150", "dhcp lease max", "dhcp"),
        ("dhcp-leasefile=/var/lib/dnsmasq/leases", "dhcp leasefile", "dhcp"),
        ("dhcp-script=/bin/echo", "dhcp script", "dhcp"),
        ("dhcp-hostsfile=/etc/dnsmasq.dhcp-hosts", "dhcp hostsfile", "dhcp"),
        ("dhcp-hostsdir=/etc/dnsmasq.dhcp-hosts.d", "dhcp hostsdir", "dhcp"),
        ("dhcp-optsfile=/etc/dnsmasq.dhcp-opts", "dhcp optsfile", "dhcp"),
        ("dhcp-optsdir=/etc/dnsmasq.dhcp-opts.d", "dhcp optsdir", "dhcp"),
        ("read-ethers", "read ethers", "dhcp"),
        ("dhcp-no-override", "dhcp no override", "dhcp"),
        ("dhcp-sequential-ip", "dhcp sequential ip", "dhcp"),
        ("dhcp-broadcast=tag:needs-broadcast", "dhcp broadcast", "dhcp"),
        ("dhcp-circuitid=set:foo,bar", "dhcp circuitid", "dhcp"),
        ("dhcp-remoteid=set:foo,bar", "dhcp remoteid", "dhcp"),
        ("dhcp-subscrid=set:foo,bar", "dhcp subscrid", "dhcp"),
        ("dhcp-client-update", "dhcp client update", "dhcp"),
        ("dhcp-fqdn", "dhcp fqdn", "dhcp"),
        ("dhcp-generate-names", "dhcp generate names", "dhcp"),
        ("dhcp-proxy=192.168.1.1", "dhcp proxy", "dhcp"),
        ("dhcp-relay=192.168.1.1,192.168.2.1", "dhcp relay", "dhcp"),
        ("dhcp-split-relay=192.168.1.0,192.168.2.1", "dhcp split relay", "dhcp"),
        ("dhcp-duid=00:00:00:01:00:01:12:34:56:78:9a:bc", "dhcp duid", "dhcp"),
        ("dhcp-ttl=3600", "dhcp ttl", "dhcp"),
        ("dhcp-scriptuser=root", "dhcp scriptuser", "dhcp"),
        ("dhcp-alternate-port=1067,1068", "dhcp alt port", "dhcp"),
        ("dhcp-ignore-clid", "dhcp ignore clid", "dhcp"),
        ("dhcp-pxe-vendor=PXEClient", "dhcp pxe vendor", "dhcp"),
        ("no-ping", "no ping", "dhcp"),
        ("log-dhcp", "log dhcp", "dhcp"),
        ("quiet-dhcp", "quiet dhcp", "dhcp"),
        ("quiet-dhcp6", "quiet dhcp6", "dhcp"),
        ("quiet-ra", "quiet ra", "dhcp"),
        ("leasefile-ro", "leasefile ro", "dhcp"),
        ("enable-ra", "enable ra", "dhcp"),
        ("ra-param=eth0,60,600", "ra param", "dhcp"),
        ("dhcp-reply-delay=2", "dhcp reply delay", "dhcp"),
        ("tag-if=set:foo,tag:bar", "tag if", "dhcp"),
        ("script-arp", "script arp", "dhcp"),
        ("script-on-renewal", "script on renewal", "dhcp"),

        // ── DHCP Option Names ──
        ("dhcp-option=option:subnet-mask,255.255.255.0", "subnet mask", "dhcp"),
        ("dhcp-option=option:domain-name,example.com", "domain name opt", "dhcp"),
        ("dhcp-option=option:ntp-server,192.168.1.1", "ntp server opt", "dhcp"),
        ("dhcp-option=option:domain-search,example.com", "domain search opt", "dhcp"),
        ("dhcp-option=option:mtu,1500", "mtu opt", "dhcp"),
        ("dhcp-option=option:static-route,10.0.0.0/8,192.168.1.1", "static route opt", "dhcp"),
        ("dhcp-option=option:classless-static-route,10.0.0.0/8,192.168.1.1", "classless route opt", "dhcp"),

        // ── DHCPv6 Settings ──
        ("dhcp-range=1234::2,1234::500,64,12h", "dhcpv6 range", "dhcp6"),
        ("dhcp-range=1234::,ra-only", "ra only range", "dhcp6"),

        // ── PXE Settings ──
        ("pxe-prompt=Press F8 for boot menu,60", "pxe prompt", "dhcp"),
        ("pxe-service=x86PC,Install Linux,pxelinux", "pxe service", "dhcp"),

        // ── TFTP Settings ──
        ("enable-tftp", "enable tftp", "tftp"),
        ("tftp-root=/var/lib/tftpboot", "tftp root", "tftp"),
        ("tftp-secure", "tftp secure", "tftp"),
        ("tftp-no-blocksize", "tftp no blocksize", "tftp"),
        ("tftp-no-fail", "tftp no fail", "tftp"),
        ("tftp-max=50", "tftp max", "tftp"),
        ("tftp-mtu=576", "tftp mtu", "tftp"),
        ("tftp-single-port", "tftp single port", "tftp"),
        ("tftp-lowercase", "tftp lowercase", "tftp"),
        ("tftp-unique-root=mac", "tftp unique root", "tftp"),
        ("tftp-port-range=10000,10100", "tftp port range", "tftp"),
        ("quiet-tftp", "quiet tftp", "tftp"),

        // ── DNSSEC Settings ──
        ("dnssec", "dnssec enable", "dnssec"),
        ("dnssec-check-unsigned", "dnssec check unsigned", "dnssec"),
        ("dnssec-no-timecheck", "dnssec no timecheck", "dnssec"),
        ("dnssec-debug", "dnssec debug", "dnssec"),
        ("dnssec-timestamp=/run/dnsmasq/dnssec-timestamp", "dnssec timestamp", "dnssec"),
        ("trust-anchor=.,20326,8,2,E06D44B80B8F1D39A95C0B0D7C65D08458E880409BBC683457104237C7F8EC8D", "trust anchor", "dnssec"),
        ("proxy-dnssec", "proxy dnssec", "dnssec"),

        // ── Auth DNS Settings ──
        ("auth-zone=example.com,eth0", "auth zone", "auth"),
        ("auth-server=ns1.example.com,eth0", "auth server", "auth"),
        ("auth-ttl=300", "auth ttl", "auth"),
        ("auth-soa=1234,hostmaster.example.com,1200,120,604800", "auth soa", "auth"),
        ("auth-sec-servers=ns2.example.com", "auth sec servers", "auth"),
        ("auth-peer=192.168.1.1", "auth peer", "auth"),
        ...
        ("enable-dbus", "enable dbus", "dbus"),
        ("enable-ubus", "enable ubus", "ubus"),
        ("conntrack", "conntrack", "conntrack"),
        ("connmark-allowlist-enable", "connmark enable", "conntrack"),
        ("connmark-allowlist=123,0xFF", "connmark allowlist", "conntrack"),

        // ── Dump/Debug Settings ──
        ("dumpfile=/tmp/dnsmasq.pcap", "dumpfile", "dumpfile"),
        ("dumpmask=1", "dumpmask", "dumpfile"),

        // ── DHCP6 Protocol Options ──
        ("leasequery", "leasequery", "dhcp"),
        ("bootp-dynamic", "bootp dynamic", "dhcp"),
    ];
    ...
    for (directive_line, description, required_feature) in &directives {
        // Skip directives whose required Cargo feature is not compiled in.
        // This allows the test to pass under default, all-features, and
        // no-default-features configurations without false negatives.
        if !required_feature.is_empty() && !feature_enabled(required_feature) {
            skipped += 1;
            continue;
        }
    ...
    if !failures.is_empty() {
        let report: String = failures
            ...
            .join("\n");
        panic!(
            "{} of {} directives failed parsing ({} skipped due to disabled features):\n{}\n\
    ...
    let tested = recognized + skipped;
    assert_eq!(
        tested as usize,
        directives.len(),
        "All {} tested directives should be recognised or skipped (recognised: {}, skipped: {})",
        directives.len(),
        recognized,
        skipped
    );
